...
select count(*) from orderTbl; #มีกี่record
select custNo as 'ลูกค้า', count(custNo) as 'จำนวนครั้งที่ซื้อ' from orderTbl group by custNo;
select avg(empCommrate) as 'ค่าเฉลี่ยคน' from empTbl;
select max(prodPrice) as 'ราคาแพงสุด', min(prodPrice) as 'ราคาถูกที่สุด', sum(prodPrice) as 'ราคารวม', avg(prodPrice) as 'ราคาถเฉลี่ย' from prodTbl;
select custNo as 'ลูกค้า', count(custNo) as 'จำนวนครั้งที่ซื้อ' from orderTbl group by custNo having count(custNo) >= 2;
